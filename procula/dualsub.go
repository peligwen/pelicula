...
func writeASS(outPath string, prof DualSubProfile, topCues []SubtitleCue, bottomTexts []string) error {
	// Derive a human-readable title from the output path (e.g. "en-es" from "Movie.en-es.ass").
	// This populates the ASS [Script Info] Title field, which Jellyfin and other players
	// display in their subtitle track picker.
	base := strings.TrimSuffix(filepath.Base(outPath), ".ass")
	pair := ""
	...
	title := "Dual Subs"
	if pair != "" {
		title = "Dual Subs (" + pair + ")"
	}

	tmp := outPath + ".partial"
	if err := writeASSContent(tmp, title, prof, topCues, bottomTexts); err != nil {
...
func writeASSContent(path, title string, prof DualSubProfile, topCues []SubtitleCue, bottomTexts []string) error {
