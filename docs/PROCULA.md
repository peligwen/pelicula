...
Dual subtitles is a **post-Bazarr** stage. The typical flow is: Bazarr acquires a `.es.srt` sidecar → Procula's dualsub stage stacks it with the existing English track into `Movie.en-es.ass`. Human-authored subtitles are the only source — there is no machine-translation fallback. When no secondary track is available after Bazarr's pass, the stage records a `dualsub_error` on the job and moves on; the job still completes. (An Argos Translate fallback existed through 2026-07 but was removed: it required an undocumented `pip install` inside the container, and machine-translated cue quality was never good enough to ship.)
...
2. Finds the secondary-language track the same way. If neither an embedded stream nor a sidecar exists, the pair fails with a recorded `dualsub_error` (non-fatal — the job completes without that sidecar).
