...
|------|------|-------------|
| `compose/docker-compose.yml` | Always | Base stack, all 11 services. Jellyfin port 8920 published on host for operator port-forwarding. |
| `compose/docker-compose.override.yml` | Non-macOS (generated by `pelicula up`) | TUN device mapping (`/dev/net/tun`) for gluetun |
| `compose/docker-compose.cloudflared.yml` | Operator opt-in | Cloudflare Tunnel sidecar — no open ports needed; set `CLOUDFLARE_TUNNEL_TOKEN` in `.env` and add to compose command manually |
| `compose/docker-compose.tailscale.yml` | Operator opt-in | Tailscale sidecar — devices on the tailnet reach nginx directly; set `TAILSCALE_AUTH_KEY` in `.env` |
...
`compose/docker-compose.override.yml` is gitignored (generated at runtime). All other overlay files are checked in.
