...
### Removed
- **Pelicula-managed remote vhost** — Pelicula no longer orchestrates an external nginx vhost, Let's Encrypt / certbot, or REMOTE_MODE settings. Pelicula listens LAN-only on port 7354; Jellyfin's built-in HTTPS on port 8920 is the supported external surface (operator port-forwards via DSM / router). Retires `nginx/remote.conf.template`, `nginx/remote-simple.conf.template`, `cmd/pelicula/remote.go`, the certbot service, and the Settings → Remote access UI. Existing `.env` files have `REMOTE_*` keys auto-stripped on next `pelicula up` (cert dirs are also cleaned up). The `compose/docker-compose.cloudflared.yml` and `compose/docker-compose.tailscale.yml` overlays remain in the repo as operator opt-ins (no longer auto-wired by REMOTE_MODE — invoke via `docker compose -f` directly if you want them).
