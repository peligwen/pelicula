...
| `docker-compose.test.yml` | ``make e2e`` only | Stubs gluetun with an Alpine nc loop; prefixes all `container_name` values with `pelicula-test-*` so the test stack coexists with a live prod stack on port 7399 |
